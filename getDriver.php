<?php
	...
	$driverQuery = "SELECT id, name, picture FROM (SELECT id, name,  FROM users WHERE isDriver = 1) as driver JOIN user_location on driver.id = user_location.user_id WHERE location = '" . $pickingPoint . "'"
	$driverResults = $db->query($driverQuery);

	$drivers = array();
	$preferredDrivers = array();
	$otherDrivers = array();
	while ($driverResults->num_rows > 0) {
		$driver = $driverResults->fetch_assoc());
